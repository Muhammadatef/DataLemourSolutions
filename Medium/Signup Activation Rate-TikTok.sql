/*
New TikTok users sign up with their emails. They confirmed their signup by replying to the text confirmation 
to activate their accounts. Users may receive multiple text messages for account confirmation until 
they have confirmed their new account.
...
WITH TikTok_email as (
SELECT emails.user_id, MAX(case when t.signup_action = 'Confirmed' then 1 else 0 end) as confirmation
FROM emails
LEFT JOIN texts t
ON emails.email_id = t.email_id
GROUP BY emails.user_id
)

select ROUND((SUM(confirmation) * 1.0) / COUNT(*), 2) as confirm_rate 
from TikTok_email;
